...
class DeepONet(pl.LightningModule):
    ...
    def on_test_end(self, num_points=100, idx_arg=0):
        with torch.no_grad():
            ...
            dataset_idx = idx_arg * datamodule.sampled

            if dataset_idx >= len(test_dataset):
                raise IndexError(
                    f"idx_arg={idx_arg} is outside the test dataset "
                    f"({len(test_dataset) // datamodule.sampled} trajectories)."
                )

            batch = test_dataset[dataset_idx]
            f_x = batch['input_func'].unsqueeze(0)  # [1, m]
            domain = self.domain
